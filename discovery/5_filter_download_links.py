...
async def classify_link_array(link_array: list[Dict[str, Any]], pub_title: str) -> list[Dict[str, Any]]:
    ...
    prompt = (
        ...
        "In general, we should download the most citable English "
        "version of any given document. PDF is more citable "
        "than HTML, which is in turn more citable than plain text. If a "
        "document is available in multiple formats, set the to_download field "
        "to true for the most citable version and false for less citable "
        "duplicates.\n"
