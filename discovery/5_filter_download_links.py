...
async def classify_link_array(link_array: list[Dict[str, Any]], pub_title: str) -> list[Dict[str, Any]]:
    ...
    prompt = (
        ...
        "In general, we should download the most easily parseable English "
        "version of any given document. Plain text is more easily parseable "
        "than HTML, which is in turn more easily parseable than PDF. If a "
        "document is available in multiple formats, set the to_download field "
        "to true for the most parseable version and false for less parseable "
        "duplicates.\n"
        "For all English-language items without duplicates, set the "
        "to_download field to true. For all non-English language items, "
        "set the to_download field to false.\n"
