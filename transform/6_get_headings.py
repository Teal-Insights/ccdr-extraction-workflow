...
2. Uses the Gemini API to identify and extract hierarchical headings (e.g., titles, section headers, figure captions) from the text content of each document.
3. Saves the results to `transform/text/headings.json` with the following structure:
   - `doc_id`: Mapped to a list of headings, each with:
     - `level`: The zero-indexed hierarchical level of the heading (e.g., 0 for document title, 1 for main sections, etc.).
     - `text`: The cleaned-up heading text.
     - `page_number`: The PDF page number (absolute, not logical) where the heading appears.

The script focuses on extracting structured headings to enable navigation and semantic analysis of the document content.
"""
