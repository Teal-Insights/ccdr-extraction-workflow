...
def check_aws_authentication() -> tuple[bool, boto3.Session | None]:
    ...
    try:
        ...
        if profile_name:
            ...
            logger.error("Please run 'aws sso login' or set AWS_PROFILE environment variable.")
...
def create_bucket_if_not_exists(bucket_name: str, region: str, session: boto3.Session) -> None:
...
def sync_data_to_s3(bucket_name: str, session: boto3.Session) -> None:
    ...
    logger.info(f"S3 sync completed. {uploaded_files} new PDF files uploaded out of {total_files - skipped_files} total PDF files.")
...
def main():
    ...
    try:
        ...
        if not auth_success or session is None:
            profile_name = os.getenv('AWS_PROFILE')
            if profile_name:
                logger.info(f"To authenticate, run: aws sso login --profile {profile_name}")
            else:
                logger.info("To authenticate, run: aws sso login (or set AWS_PROFILE and run aws sso login --profile <profile_name>)")
            return 1
