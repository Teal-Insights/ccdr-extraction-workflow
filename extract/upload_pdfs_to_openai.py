...
async def main():
    ...
    logger.info(
        f"Found vector store ID: {vector_store_id} for assistant {assistant_id}"
    )
    ...
    if not pdf_files:
        logger.info(
            "No PDF files found in extract/data subdirectories. Nothing to upload."
        )
        return
    ...
    if not files_to_upload:
        logger.info(
            "All PDF files already exist in the vector store. Nothing to upload."
        )
        return
    ...
    try:
        ...
        logger.info(
            f"File Counts: Total={file_batch.file_counts.total}, Completed={file_batch.file_counts.completed}, Failed={file_batch.file_counts.failed}, Cancelled={file_batch.file_counts.cancelled}, InProgress={file_batch.file_counts.in_progress}"
        )
